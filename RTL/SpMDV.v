...
(
    ...
    // ============================================================
    // 16-MAC architecture: 4 BBS matrix banks x 4 dense vectors.
    ...
    // computes only 4 feature lanes at a time. There are 4
    ...
    // Dense vectors: 4 active feature lanes x 4 column banks = 16 SRAMs.
    ...
    // 4 feature lanes x 3 result bytes = 12 SRAMs.
    // The block is output before computing the next feature block.
    reg        result_chip_enable [0:11];
    reg        result_write_enable[0:11];
    reg [7:0]  result_address     [0:11];
    reg [7:0]  result_data        [0:11];
    wire [7:0] result_output      [0:11];
    ...
    generate
        ...
        for (gf = 0; gf < 4; gf = gf + 1) begin : GEN_VECTOR_LANE
        ...
        for (gf = 0; gf < 4; gf = gf + 1) begin : GEN_RESULT_LANE
    ...
    // ============================================================
    reg [7:0] row;
    reg [1:0] feature_block;
    reg [7:0] output_row;
    reg [10:0] output_issue_count;
    reg [10:0] output_sent_count;
    reg        out_valid_1, out_valid_2;
    reg [1:0] out_lane_1, out_lane_2;
    ...
    reg signed [17:0] partial_sum_pipe[0:3];
    reg signed [21:0] acc[0:3];
        ...
        if (rst) begin
            ...
            case (state)
                ...
                S_START_READ_VECTOR: begin
                    ...
                    load_elem_id <= 8'd0;
`ifdef DEBUG_SPMDV
                    $display("[SpMDV_16] start loading 16 dense vectors at t=%0t", $time);
`endif
                end

                S_READ_VECTOR: begin
                    if (raw_data_valid) begin
                        vector_chip_enable[{load_feature_id[1:0], load_elem_id[7:6]}]  <= 1'b1;
                        vector_write_enable[{load_feature_id[1:0], load_elem_id[7:6]}] <= 1'b1;
                        vector_address[{load_feature_id[1:0], load_elem_id[7:6]}]      <= {load_feature_id[3:2], load_elem_id[5:0]};
                        vector_data[{load_feature_id[1:0], load_elem_id[7:6]}]         <= raw_input;
                    ...
                    if (mat_valid_2) begin
                        ...
                        for (f = 0; f < 4; f = f + 1) begin
                            for (b = 0; b < 4; b = b + 1) begin
                                ...
                                vector_address[f*4 + b]      <= {feature_block[1:0], position_output[b][5:0]};
                            end
                        end
                    end
...
`ifdef DEBUG_SPMDV
                        if ((out_row_2 == 8'd0) || (out_row_2 == 8'd255))
                            $strobe("[SpMDV_16] output block=%0d lane=%0d feature=%0d row=%0d result=%0d raw=%h t=%0t",
                                    feature_block, out_lane_2, {feature_block[1:0], out_lane_2}, out_row_2,
...
`endif
                    ...
                    if (output_issue_count < 11'd1024) begin
                        result_chip_enable[output_issue_count[9:8]*3 + 0] <= 1'b1;
                        result_write_enable[output_issue_count[9:8]*3 + 0] <= 1'b0;
                        result_address[output_issue_count[9:8]*3 + 0] <= output_issue_count[7:0];

                        result_chip_enable[output_issue_count[9:8]*3 + 1] <= 1'b1;
                        result_write_enable[output_issue_count[9:8]*3 + 1] <= 1'b0;
                        result_address[output_issue_count[9:8]*3 + 1] <= output_issue_count[7:0];

                        result_chip_enable[output_issue_count[9:8]*3 + 2] <= 1'b1;
                        result_write_enable[output_issue_count[9:8]*3 + 2] <= 1'b0;
                        result_address[output_issue_count[9:8]*3 + 2] <= output_issue_count[7:0];

                        out_valid_1 <= 1'b1;
                        out_lane_1 <= output_issue_count[9:8];
                    ...
                    if (out_valid_2 && output_sent_count == 11'd1023) begin
                        if (!(feature_block == 2'd3)) begin
                            feature_block <= feature_block + 2'd1;
                            row <= 8'd0;
                        end
                    end
                end
    ...
    always @(*) begin
        ...
        case (state)
            ...
            S_OUTPUT_PIPE:
                if (output_sent_count == 11'd1024) begin
                    if (feature_block == 2'd3) next_state = S_START_READ_VECTOR;
